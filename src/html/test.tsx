...
export class Test extends JSXWidget {
    ...
    update(domNode, _element) {
        const jsx = <div>
            <h1>{this.title}</h1>
            <i>{this.summary}</i>
            <p><VizComponent type={Pie} columns={this.columns()} data={this.data()} style="height:480px" /></p>
            <i>{this.summary}</i>
            <p><VizComponent type={Column} columns={this.columns()} data={this.data()} yAxisDomainLow="0" yAxisDomainHigh="200" style="height:480px" /></p>
            <i>{this.summary}</i>
        </div>;
