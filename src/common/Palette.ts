...
function palette_rainbow(id?, _colors?, _steps?) {
    ...
    const _custom = function (colors2, steps?) {
        steps = steps || 32;
        const subPaletteSize = Math.ceil(steps / (colors2.length - 1));
        const range = [];
        let prevColor = null;
        colors2.forEach(function (color) {
            if (prevColor) {
                const scale2 = d3ScaleLinear()
    ...
    rainbow.switch = function (_id, colors2) {
        if (id === _id) {
            return this;
        }
        return arguments.length ? fetchRainbowItem(_id, colors2) : fetchRainbowItem();
    };
