...
(function (root, factory) {
    ...
    function initializeMarshaller(_marshaller) {
        marshaller = _marshaller;

        //  Hook vizEvents + commsEvents  ---
        marshaller
            ...
            .on("commsEvent", function (ddlSource, eventID, request, response) {
                switch (eventID) {
                    case "request":
                    ...
                    case "response":
                        ...
                        break;
                    case "error":
                        alert("Marshaller.commsEvent:  " + ddlSource.id + "-" + eventID + ":  " + JSON.stringify(response));
                        break;
                    default:
        ...
        //  Hook populateContent (so we can specify "bootstrap" location on web page)  ---
        ...
        marshaller.populateContent = function () {
            ...
            this.sortedContent().forEach(function (cell) {
                ...
                //  Custom widget (graph/edge) handling  ---
                switch (cellWidget.classID()) {
                    //  HipieDDL creates graphs in a regular Surface - most widgets are created in a MegaChart  ---
                    case "layout_Surface":
                        var graphWidget = cellWidget.widget();
                        //  Is widget an instance of "Graph"  ---
                        if (graphWidget && graphWidget.classID() === "graph_Graph") {
                            //  Hook "data" call so we can set additional properties to the edges (links)  ---
                            var origData = graphWidget.data;
                            graphWidget.data = function (_) {
                                var retVal = origData.apply(this, arguments);
                                if (arguments.length && _.edges) {
                                    _.edges.forEach(function (edge) {
                                        //  Set additional properties for the edges (links)  ---
                ...
                //  Place the cell into the bootstrap grid  ---
                cell
                    .target(cellDiv.node())
                ;
            });
        ...
        marshaller
            .render(function () {
                //  Render no longer does anything as we have removed all the widgets and placed them inside the bootstrap grid  ---
                //  Manually call render content  ---
                marshaller.renderContent(function () {
                    //  Apply theme  ---
                    Persist.applyTheme(marshaller, { "common_Class": { "surfacePadding": "0", "surfaceBorderWidth": 1, "titleFontColor": "#f3f2f7", "titleBackgroundColor": "#3d3045", "surfaceBackgroundColor": "#e1dee7", "theadFontColor": "#f3f2f7", "theadCellBorderColor": "white", "theadRowBackgroundColor": "#3d3045", "tableZebraColor": "#f3f2f7" }, "common_PropertyExt": {}, "map_Choropleth": { "paletteID": "Purples" } })

                    //  Yet another renderContent for theme to display (this could be improved, so we can set the theme prior to first render)  ---
                    marshaller.renderContent(function () {

                        //  Unhide the entire dashborad  ---
                        mainDiv.transition()
                            .style("opacity", 1)
                        ;
                    });

                    //  Update next/previous history  ---
                    updateHistory();
                });
            })
        ;
