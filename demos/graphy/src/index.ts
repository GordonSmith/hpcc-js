...
export class App extends SplitPanel {
    ...
    private _graph: CoreGraph | IMDBGraph;
    ...
    refreshTables(render: boolean = false) {
        const movies = [];
        const people = [];
        const data = this._graph.data();
        data.vertices && data.vertices.forEach(v => {
    ...
    enter(domNode, element) {
        ...
        this._graph = !!this._root ? new CoreGraph() : new IMDBGraph();

        this._graphPanel.widget(this._graph as any);
