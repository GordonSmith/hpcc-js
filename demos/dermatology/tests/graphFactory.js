...
(function (root, factory) {
    ...
    var DataGraphVerticesBig = [];
    var DataGraphEdgesBig = [];
    for (var i = 0; i < 500; ++i) {
        DataGraphVerticesBig.push([i, "Node-" + i, 1]);
        if (i > 0) {
            DataGraphEdgesBig.push([("0->" + i), 0, i, 1]);
        }
    }
    ...
    return {
        Graph2: {
            simple: function (callback) {
                legacyRequire(["src/graph/Graph2"], function (Graph2) {
                    let vertices = [
                        { id: 0, text: "Daddy" },
                        { id: 1, text: "Mummy" },
                        { id: 2, text: "Baby" }
                    ...
                    setTimeout(() => {
                        ...
                        edges = [
                            { id: 0, source: vertices[0], target: vertices[2] },
                            { id: 1, source: vertices[1], target: vertices[2] },
                            { id: 2, source: vertices[0], target: vertices[3] },
                            { id: 3, source: vertices[1], target: vertices[3] }
                        ];
            ...
            LesMis_1: function (callback) {
                legacyRequire(["src/graph/DataGraph"], function (DataGraph) {
                    const graph = new DataGraph()
                        .vertexColumns(["id", "label", "group"])
                        .vertexIDColumn("id")
                        .vertexLabelColumn("label")
                        .vertices(DataGraphVerticesBig)
                        .edgeColumns(["id", "source", "target", "weight"])
                        .edgeIDColumn("id")
                        .edgeSourceColumn("source")
                        .edgeTargetColumn("target")
                        .edges(DataGraphEdgesBig)
                        ;
                    callback(graph);
                });
            },
            LesMis_2: function (callback) {
                legacyRequire(["src/graph/DataGraph2"], function (DataGraph2) {
                    const graph = new DataGraph2()
                        .vertexColumns(["id", "label", "group"])
                        .vertexIDColumn("id")
                        .vertexLabelColumn("label")
                        .vertices(DataGraphVerticesBig)
