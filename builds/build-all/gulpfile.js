...
gulp.task("copy-json", shell.task([
    ...
    cpx.copySync("node_modules/@hpcc-js/requirejs-shim/dist/*", "dist")
]));

var cache;
function doRollup(libName, entry, dest, format, external, _alias) {
    return Promise.all([
        _doRollup(libName, entry, dest, format, false, external, _alias),
        _doRollup(libName, entry, dest, format, true, external, _alias)
    ]);
}

function _doRollup(libName, entry, dest, format, min, external, _alias) {
    console.log(dest);
